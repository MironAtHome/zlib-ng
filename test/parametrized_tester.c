/* parametrized_tester.c - Test deflate() and inflate() on a chunk of the
...
int main (int argc, char *argv[])  {
    PREFIX3(stream) zst_deflate;
    PREFIX3(stream) zst_inflate;
    ...
    if ((argc == 2 && strcmp(argv[1], "--help") == 0)) {
        printf("Usage: test_parametrized <FILENAME>\n\n");
        return 0;
    }
        ...
        for (int complevel_index=0; complevel_index < NUM_C; complevel_index+=1) {
            for (int memlevel_index=0; memlevel_index < NUM_M; memlevel_index+=1) {
                for (int strat_index=0; strat_index < 5; strat_index+=1) {
                    ...
                    memset(&zst_deflate, 0, sizeof(PREFIX3(stream)));
                    memset(&zst_inflate, 0, sizeof(PREFIX3(stream)));

                    int err = PREFIX(deflateInit2)(&zst_deflate, level, Z_DEFLATED, wbits, memlevel, strategy);
                    if (err != Z_OK) {
                        fprintf(stderr, error_format, "deflateInit2", level, wbits, memlevel, strategy, zst_deflate.msg);
                        return EXIT_FAILURE;
                    }
                    ...
                    zst_deflate.avail_out = MAX_OUTSIZE;

                    err = PREFIX(deflate)(&zst_deflate, Z_FINISH);
                    if (err != Z_STREAM_END) {
                        fprintf(stderr, error_format, "deflate", level, wbits, memlevel, strategy, zst_deflate.msg);
                        return EXIT_FAILURE;
                    }

                    err = PREFIX(inflateInit2)(&zst_inflate, wbits);
                    if (err != Z_OK) {
                        fprintf(stderr, error_format, "inflateInit2", level, wbits, memlevel, strategy, err, zst_inflate.msg);
                        return EXIT_FAILURE;
                    }

                    zst_inflate.next_in = compressed_out;
                    zst_inflate.avail_in = MAX_OUTSIZE - zst_deflate.avail_out;
                    zst_inflate.next_out = decompressed_out;
                    zst_inflate.avail_out = MAX_OUTSIZE;

                    err = PREFIX(inflate)(&zst_inflate, Z_FINISH);
                    if (err != Z_STREAM_END) {
                        fprintf(stderr, error_format, "inflate", level, wbits, memlevel, strategy, err, zst_inflate.msg);
                        return EXIT_FAILURE;
                    }

                    if (((MAX_OUTSIZE - zst_inflate.avail_out) != read_size) || memcmp(data_in, decompressed_out, read_size) !=0) {
                        fprintf(stderr, error_format, "Input and output is not the same", level, wbits, memlevel, strategy, 0, "");
                        fprintf(stderr,
                                "in bytes: %u\n"
                                "out_bytes:%u\n"
                                "compare_result:%d\n",
                                read_size, MAX_OUTSIZE - zst_inflate.avail_out,
                                memcmp(data_in, decompressed_out, read_size));
                        return EXIT_FAILURE;
                    }

                    err = PREFIX(deflateEnd)(&zst_deflate);
                    if (err != Z_OK) {
                        printf("%d", zst_deflate.avail_out);
                        fprintf(stderr, error_format, "deflateEnd", level, wbits, memlevel, strategy, err, zst_deflate.msg);
                        return EXIT_FAILURE;
                    }

                    err = PREFIX(inflateEnd)(&zst_inflate);
                    if (err != Z_OK) {
                        fprintf(stderr, error_format, "inflateEnd", level, wbits, memlevel, strategy, err, zst_inflate.msg);
                        return EXIT_FAILURE;
                    }
                }
            }
